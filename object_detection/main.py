...
def main():
    try:
        # camera = ZEDCamera()
        camera = RealSenseCamera(simulate=True)
        ...
        while True:
            ...
            coordinates = detector.get_pixel_coordinates(results)
            output = detector.draw_boxes(results, image.copy())
            for object_coordinates in coordinates:
            ...
            cv2.imshow('MediaPipe Objectron', output)
