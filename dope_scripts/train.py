...
def show_belief_maps(target_belief, output_belief, epoch):
    imgs = VisualizeBeliefMap(target_belief[0])
    img, grid = save_image(imgs, "some_img.png", 
        mean=0, std=1, nrow=3, save=False)
    writer.add_image("train_belief_ground_truth_{}".format(epoch),
                        grid,
                        epoch, 
                        dataformats="CWH")
    # belief maps guess
    imgs = VisualizeBeliefMap(output_belief[-1][0])
    img,grid = save_image(imgs, "some_img.png", 
        mean=0, std=1, nrow=3, save=False)
    writer.add_image("train_belief_guess_{}".format(epoch),
                grid,
                epoch, 
                dataformats="CWH")


def log_losses(loss_avg_to_log, epoch):
...
def _runnetwork(epoch, loader, train=True):
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        if batch_idx == 0:
            show_belief_maps(target_belief, output_belief, epoch)
        ...
        if batch_idx % opt.loginterval == 0:
            log_losses(loss_avg_to_log, epoch)
